...
`const` は、知ってのとおり**定数**を宣言するキーワードだ。今回は `MyButton` という名前の定数を宣言している。これもよ～く見てみると `const 変数名: 型アノテーション = 処理` という形になっているはずだ。 +
そしてその型は `React.FC<Props>` となっている。

これは**関数コンポーネント**と呼ばれる手法だ。事実、`React.FC<P>` は `React.FunctionalComponent<P>` のエイリアスである。
...
とは言っても中身は TypeScript なので、それに合わせ `onClick` プロパティも camelCase になっている。`class` プロパティも TypeScript の予約語なので `className` に変えられていたりする。
